...
//----------------
void VisualizeOrientation(k4abt_frame_t bodyFrame, Window3dWrapper& window3d, int depthWidth, int depthHeight) {
    ...
    //std::vector<std::reference_wrapper<const k4a_quaternion_t&>> NavelJointOrientations;
    std::vector<k4abt_body_t> Bodies;
    std::vector<k4a_float3_t> NavelJoints;
    std::vector<k4a_quaternion_t> NavelJointOrientations;
    //const k4a_float3_t NavelJoints[numBodies];
    //const k4a_quaternion_t NavelJointOrientations[numBodies];
    ...
    for (uint32_t i = 0; i < numBodies; i++)
    {
        ...
        {
            const k4a_float3_t& jointPosition = body.skeleton.joints[K4ABT_JOINT_SPINE_NAVEL].position;
            const k4a_quaternion_t& jointOrientation = body.skeleton.joints[K4ABT_JOINT_SPINE_NAVEL].orientation;

            NavelJoints.push_back(jointPosition);
            NavelJointOrientations.push_back(jointOrientation);

            window3d.AddJoint(
                jointPosition,
                jointOrientation,
                body.skeleton.joints[K4ABT_JOINT_SPINE_NAVEL].confidence_level >= K4ABT_JOINT_CONFIDENCE_MEDIUM ? color : lowConfidenceColor);
    ...
    if (numBodies > 1)
    {
        ...
        float NavelsDistance_Y;
        float NavelsDistance_Z;
        float NavelsDistance;
        NavelsDistance_X = pow(Bodies.at(numBodies-1).skeleton.joints[K4ABT_JOINT_SPINE_NAVEL].position.xyz.x - Bodies.at(numBodies - 2).skeleton.joints[K4ABT_JOINT_SPINE_NAVEL].position.xyz.x, 2);
        NavelsDistance_Y = pow(Bodies.at(numBodies-1).skeleton.joints[K4ABT_JOINT_SPINE_NAVEL].position.xyz.y - Bodies.at(numBodies - 2).skeleton.joints[K4ABT_JOINT_SPINE_NAVEL].position.xyz.y, 2);
        NavelsDistance_Z = pow(Bodies.at(numBodies-1).skeleton.joints[K4ABT_JOINT_SPINE_NAVEL].position.xyz.z - Bodies.at(numBodies - 2).skeleton.joints[K4ABT_JOINT_SPINE_NAVEL].position.xyz.z, 2);
    ...
    while (playbackResult == K4A_STREAM_RESULT_SUCCEEDED && s_isRunning)
    {
        ...
        if (playbackResult == K4A_STREAM_RESULT_SUCCEEDED)
        {
            ...
            if (popFrameResult == K4A_WAIT_RESULT_SUCCEEDED)
            {
                ...
                VisualizeOrientation(bodyFrame, window3d, depthWidth, depthHeight); // SARAH
